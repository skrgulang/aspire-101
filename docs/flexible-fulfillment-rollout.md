...
Financial serialization now has both committed-order and genuine multi-session validation. For refund-vs-label, two real concurrent PostgreSQL-session races were executed against independent shipping orders. In the refund-first race, refund and label transactions started within milliseconds; refund held the protected payment lock and committed, while label purchase waited and then failed with `REFUND_IN_PROGRESS`. In the label-first race, label purchase acquired the shared payment-first boundary and committed `label_purchasing`; the concurrent refund then failed with `SHIPPING_REFUND_REQUIRES_RESOLUTION`. For payout-vs-hold, two real concurrent PostgreSQL-session races were also executed. In the release-first run, release claimed and held the protected payment row while the Resolution Center transaction blocked, then failed with `PAYOUT_RELEASE_IN_PROGRESS` after release committed. In the reverse run, the Resolution Center hold acquired the same payment-row boundary first; the delayed concurrent release then failed with `PAYOUT_HOLD_OPEN`. Protected cancellation also remains serialized: release-first blocks cancellation with `PAYOUT_RELEASE_IN_PROGRESS`, while cancellation-first creates the required review and prevents a later release claim with `CONNECTION_CANCELLED`.

The preview-only `pg_cron`/`dblink` test extensions were used only to create independent database sessions for concurrency validation; no such test-only extension migration was added to PR #91 or production. All scheduled concurrency jobs were unscheduled, synthetic users/requests were removed, and the test-only extensions were removed from the preview branch after the concurrency matrix completed.
...
- [x] True concurrent refund-vs-label DB claim race: refund-first makes label fail with `REFUND_IN_PROGRESS`; label-first makes refund fail with `SHIPPING_REFUND_REQUIRES_RESOLUTION`. External Shippo transaction behavior remains covered by the separate carrier E2E gate.
