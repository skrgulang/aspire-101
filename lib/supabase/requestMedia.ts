...
function friendlyRequestMediaError(error: { message?: string; details?: string; hint?: string }) {
  ...
  if (/row-level security|permission denied for function can_upload_|policy/i.test(detail)) return new Error('Aspire could not attach that photo to this post. Please try again.');
