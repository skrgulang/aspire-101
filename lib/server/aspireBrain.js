...
function inferExplicitMarketplaceIntent(text) {
  const sell = /\b(wts|sell|selling|for sale|listing)\b/.test(text);
  const wantedStrong = /\b(wtb|iso|wanted)\b/.test(text);
  const itemWord = /\b(marketplace|textbook|book|bike|laptop|monitor|desk|chair|calculator|fridge|furniture)\b/.test(text);
  const wantedWithItem = itemWord && /\b(buy|buying|purchase|shopping for)\b/.test(text);
  const wanted = wantedStrong || wantedWithItem;
  if (sell && !wanted) return 'sell';
  if (wanted && !sell) return 'wanted';
  return null;
}
...
export function inferIntentHints(message = '') {
  ...
  let category = null;
  let kind = null;
  let market_intent = inferExplicitMarketplaceIntent(text);

  // Only high-confidence transaction language outranks overlapping subject words.
  // This fixes "selling my math textbook" without turning generic errands into marketplace intent.
  if (market_intent) { category = 'Buy & sell'; kind = 'buy_sell'; }
  else if (/\b(ride|carpool|drive|airport|uber|lyft)\b/.test(text)) { category = 'Ride'; kind = 'split_cost'; }
  ...
  if (kind === 'buy_sell' && !market_intent) {
