...
export function inferIntentHints(message = '') {
  ...
  const marketplaceItem = /\b(textbook|book|bike|bicycle|laptop|monitor|desk|chair|phone|furniture)\b/.test(text);
  // High-confidence marketplace language should beat incidental subject words
  // (for example, "selling my math textbook" should not become Study).
  const explicitSell = /\b(sell|selling|for sale|wts)\b/.test(text);
  const explicitWanted = /\b(wtb|iso|wanted)\b/.test(text) || (/\b(buying|purchase)\b/.test(text) && marketplaceItem);
  const explicitBuyItem = /\bbuy\b/.test(text) && marketplaceItem;
  const highConfidenceMarketplace = explicitSell || explicitWanted || explicitBuyItem;

  // Person-to-person errands such as "can someone buy groceries for me" are
  // deterministic campus-help requests, not marketplace purchases.
  const asksAnotherPerson = /\b(can|could|would|will)\s+(?:someone|somebody|anyone|you)\b|\b(?:someone|somebody|anyone)\s+(?:can|could|please)\b/.test(text);
  const errandObject = /\b(groceries|grocery|food|medicine|meds|supplies)\b/.test(text);
  const errandAction = /\b(buy|grab|get|pick up|pickup|fetch|deliver|delivery)\b/.test(text);
  const delegatedErrand = asksAnotherPerson && errandObject && errandAction;

  if (/\b(ride|carpool|drive|airport|uber|lyft)\b/.test(text)) { category = 'Ride'; kind = 'split_cost'; }
  else if (delegatedErrand || /\b(pickup|pick up|errand|grocery|groceries|delivery|fetch)\b/.test(text)) { category = 'Pickup / errand'; kind = 'paid_help'; }
  else if (/\b(move|moving|furniture|carry|lift|搬家)\b/.test(text)) { category = 'Moving / help'; kind = 'paid_help'; }
  else if (highConfidenceMarketplace) { category = 'Buy & sell'; kind = 'buy_sell'; }
  ...
  else if (/\b(marketplace|textbook|bike|bicycle|laptop|monitor|desk|chair|phone|wtb|wts|iso)\b/.test(text)) { category = 'Buy & sell'; kind = 'buy_sell'; }

  if (kind === 'buy_sell') {
    if (explicitSell) market_intent = 'sell';
    else if (explicitWanted || /\b(looking for)\b/.test(text) || explicitBuyItem) market_intent = 'wanted';
  }
  ...
  if (kind === 'buy_sell') {
    if (/\b(?:brand new|new in box|nib|sealed|unopened)\b/.test(text)) item_condition = 'new';
    else if (/\b(?:like new|lnib|mint)\b/.test(text)) item_condition = 'like_new';
    else if (/\b(?:for parts|parts only|not working|broken)\b/.test(text)) item_condition = 'for_parts';
    else if (/\bfair(?: condition)?\b/.test(text)) item_condition = 'fair';
    else if (/\bgood(?: condition)?\b/.test(text)) item_condition = 'good';
  }

  let price_negotiable = null;
  if (kind === 'buy_sell') {
    if (/\b(?:obo|or best offer|negotiable|open to offers?)\b/.test(text)) price_negotiable = true;
    else if (/\b(?:firm price|price firm|firm on price|non[- ]negotiable|not negotiable)\b/.test(text)) price_negotiable = false;
  }

  // Preserve only explicit time/place phrases as hints. These are intentionally
  // conservative: Brain should never infer a time or location the student did not say.
  const timeMatch = String(message).match(/\b(today|tonight|tomorrow|this (?:morning|afternoon|evening)|(?:mon|tues|wednes|thurs|fri|satur|sun)day|(?:after|before|around|at)\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\b/i);
  const time_text = timeMatch?.[0]?.trim() || '';

  const placeMatch = String(message).match(/\b(?:at|near|by|from|to)\s+([A-Z][A-Za-z0-9.'’-]*(?:\s+[A-Z][A-Za-z0-9.'’-]*){0,3})\b/);
  const place_text = placeMatch?.[1]?.trim() || '';

  return { category, kind, market_intent, amount_cents, item_condition, price_negotiable, time_text, place_text };
}
