...
function inferExplicitMarketplaceIntent(text) {
  ...
  const wanted = /\b(wtb|iso|buy|buying|wanted|purchase|shopping for)\b/.test(text);
...
export function inferIntentHints(message = '') {
  ...
  // Explicit transaction language is higher-confidence than overlapping subject words.
  // This prevents phrases like "selling my math textbook" from being classified as Study.
  ...
  else if (/\b(marketplace|textbook|bike|laptop|monitor|desk|chair)\b/.test(text)) { category = 'Buy & sell'; kind = 'buy_sell'; }
