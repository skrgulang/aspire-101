...
export function parseAspireIntent(message: string): AspireIntent {
...

  {
    ...
    if (hit(value, /\b(?:can|could|would) someone\s+(?:please\s+)?(?:buy|grab|get|bring)\b.{0,40}\b(?:groceries|grocery|food|medicine|meds|supplies)\b/i)
      || hit(value, /\b(?:buy|grab|get|bring)\b.{0,40}\b(?:groceries|grocery|food|medicine|meds|supplies)\b.{0,25}\bfor me\b/i)) {
      score += 95; signals.push('purchase-errand');
    }
