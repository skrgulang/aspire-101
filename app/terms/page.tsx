...
const toc = [
  ['scope','Scope & acceptance'],['role','What Aspire is'],['eligibility','Eligibility & accounts'],['conduct','User conduct'],['connections','Responses & mutual connections'],['requests','Requests, meetups & safety'],['reputation','Reviews & trust signals'],['payments','Payments'],['content','Content & IP'],['third','Third-party services'],['liability','Disclaimers & liability'],['changes','Changes & termination'],['law','Law & disputes'],['contact','Contact']
];
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      <section className={styles.hero}>
        ...
        <div className={styles.heroMeta}><p><strong>Last updated:</strong> September 12, 2026</p><p>These Terms govern access to Aspire’s request, discovery, mutual matching, messaging, trust, payment, and community features.</p></div>
      </section>
      ...
      <div className={styles.content}>
        ...
        <article className={styles.doc}>
          ...
          <section id="eligibility" className={styles.section}><h2>3. Eligibility & accounts</h2><ul><li>You must satisfy the minimum age required by applicable law and any age requirement shown during registration.</li><li>Provide accurate information and protect your login credentials.</li><li>You are responsible for activity under your account.</li><li>Aspire may use account-level trust signals across requester and responder activity because users are not assigned a permanent worker or customer role.</li><li>We may limit or suspend access when reasonably necessary for safety, compliance, fraud prevention, or enforcement of these Terms.</li></ul></section>
          ...
          <section id="reputation" className={styles.section}><h2>7. Reviews & trust signals</h2><p>Aspire may let users provide feedback after eligible completed connections. Trust signals may include completed-connection counts, experience tags, account or verification status, and a “would connect again” style signal. Aspire does not need to turn those signals into one public student-wide star score.</p><ul><li>Feedback must reflect genuine interactions and may not be purchased, fabricated, manipulated, extorted, or used for retaliation.</li><li>Aspire may use blind or delayed feedback publication to reduce retaliatory behavior.</li><li>Safety reports are separate from ordinary experience feedback. A negative experience signal is not a substitute for reporting serious misconduct.</li><li>We may remove feedback or adjust trust systems when necessary to address fraud, abuse, manipulation, or product integrity.</li></ul></section>

          <section id="payments" className={styles.section}><h2>8. Payments</h2><p>Some requests, connections, or marketplace orders can use in-product payment features. Where Aspire offers “Pay with Aspire” or “Aspire Protected,” a third-party payment provider such as Stripe processes the payment, while Aspire records transaction status and may delay a separate provider or seller transfer until the applicable completion or handoff conditions are met.</p><p>Cancellation, refund, payout, and dispute treatment depends on the transaction state and the terms shown for that flow. Additional rules may be described in our <a href="/resolution-policy">Resolution Policy</a> or <a href="/marketplace-rules">Marketplace Rules</a>. Payments made outside Aspire are not processed or protected as Aspire payments. Aspire is not a bank, stored-value wallet, employer, or legal escrow service merely because a payment is marked secured or protected.</p></section>
          ...
          <section id="liability" className={styles.section}><h2>11. Disclaimers & liability</h2><p>Aspire is provided on an “as is” and “as available” basis to the maximum extent permitted by law. We do not guarantee that every request will receive a response, that another user is reliable or qualified, that every trust signal is perfectly accurate, or that an interaction will achieve a particular result.</p><p>To the maximum extent permitted by applicable law, Aspire is not responsible for losses arising from arrangements, transactions, rides, meetups, services, property exchanges, or other offline interactions between users. Nothing in these Terms limits rights or liabilities that cannot legally be excluded.</p></section>
          ...
          <p className={styles.note}>These Terms reflect Aspire’s current request, response, mutual connection, messaging, trust, marketplace, payment, and Resolution Center model. Final production terms should be reviewed by qualified counsel for the jurisdictions where Aspire operates.</p>
        </article>
      </div>
    </main>
  </div>;
