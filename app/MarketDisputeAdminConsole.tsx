          ...
          {visible.map((dispute) => {
            ...
            return (
              <article className={styles.card} key={dispute.id}>
                ...
                {open && <div className={styles.composer}><label htmlFor={`admin-dispute-${dispute.id}`}>Reply or leave a staff note</label><textarea id={`admin-dispute-${dispute.id}`} rows={3} maxLength={2000} value={drafts[dispute.id] || ''} onChange={(event) => setDrafts((current) => ({ ...current, [dispute.id]: event.target.value }))} placeholder="Write a factual update. Internal notes are never shown to participants." /><div><button type="button" onClick={() => void queueAction(dispute.id, 'assign_self')} disabled={busy === `assign_self-${dispute.id}`}>Assign to me</button><button type="button" onClick={() => void queueAction(dispute.id, 'internal_note')} disabled={!drafts[dispute.id]?.trim() || busy === `internal_note-${dispute.id}`}>Save internal note</button><button className={styles.reply} type="button" onClick={() => void queueAction(dispute.id, 'staff_reply')} disabled={!drafts[dispute.id]?.trim() || busy === `staff_reply-${dispute.id}`}>Reply to both sides</button>{order?.status === 'released' && data.role === 'admin' && <button type="button" onClick={() => void queueAction(dispute.id, 'close_after_sales')} disabled={busy.includes(dispute.id) || (drafts[dispute.id] || '').trim().length < 10}>Close after-sales review with this note</button>}</div></div>}
