...
type Order = { id: string; connection_id: string; request_id: string; buyer_id: string; seller_id: string; status: string; released_at: string | null; fulfillment_method: string; currency: string; agreed_amount_cents: number; seller_handed_off_at: string | null; buyer_received_at: string | null; admin_release_authorized_at: string | null; shipping_status: string | null };
...
export default function MarketDisputeAdminConsole() {
  ...
  async function queueAction(disputeId: string, action: 'assign_self' | 'staff_reply' | 'internal_note' | 'close_after_sales') {
    ...
    try {
      ...
      setNotice(action === 'assign_self' ? 'Case assigned and marked under review.' : action === 'staff_reply' ? 'Reply sent to both participants.' : action === 'close_after_sales' ? 'After-sales review closed without a financial action.' : 'Internal note saved for staff only.');
      await reload();
    } catch (error) {
          ...
          {visible.map((dispute) => {
            ...
            return (
              <article className={styles.card} key={dispute.id}>
                ...
                {open && (order?.status === 'released' || payment?.status === 'released') && <div className={styles.warning}><b>Seller payout already released</b><span>Do not use protected order refund or payout controls. Review both sides’ evidence, explain any proposed remedy, and handle any payment recovery through a separately reviewed process.</span></div>}
                ...
                {open && <div className={styles.composer}><label htmlFor={`admin-dispute-${dispute.id}`}>Reply or leave a staff note</label><textarea id={`admin-dispute-${dispute.id}`} rows={3} maxLength={2000} value={drafts[dispute.id] || ''} onChange={(event) => setDrafts((current) => ({ ...current, [dispute.id]: event.target.value }))} placeholder="Write a factual update. Internal notes are never shown to participants." /><div><button type="button" onClick={() => void queueAction(dispute.id, 'assign_self')} disabled={busy === `assign_self-${dispute.id}`}>Assign to me</button><button type="button" onClick={() => void queueAction(dispute.id, 'internal_note')} disabled={!drafts[dispute.id]?.trim() || busy === `internal_note-${dispute.id}`}>Save internal note</button><button className={styles.reply} type="button" onClick={() => void queueAction(dispute.id, 'staff_reply')} disabled={!drafts[dispute.id]?.trim() || busy === `staff_reply-${dispute.id}`}>Reply to both sides</button>{order?.status === 'released' && order.released_at && dispute.source === 'user' && new Date(dispute.created_at).getTime() >= new Date(order.released_at).getTime() && data.role === 'admin' && <button type="button" onClick={() => void queueAction(dispute.id, 'close_after_sales')} disabled={busy.includes(dispute.id) || (drafts[dispute.id] || '').trim().length < 10}>Close after-sales review with this note</button>}</div></div>}
                {open && data.role === 'admin' && order?.status !== 'released' && payment?.status !== 'released' && <div className={styles.decisionPanel}><label><span>Partial refund amount</span><div><b>$</b><input type="number" min="0.01" max={(remaining / 100).toFixed(2)} step="0.01" inputMode="decimal" value={refundAmounts[dispute.id] || ''} onChange={(event) => setRefundAmounts((current) => ({ ...current, [dispute.id]: event.target.value }))} placeholder={(remaining / 200).toFixed(2)} /></div></label><div className={styles.decisions}><button type="button" onClick={() => void resolve(dispute, 'resume_seller')} disabled={busy.includes(dispute.id)}>Authorize seller release</button><button type="button" onClick={() => void resolve(dispute, 'refund_buyer', partialCents)} disabled={busy.includes(dispute.id) || partialCents < 1 || partialCents >= remaining}>Issue partial refund</button><button className={styles.refund} type="button" onClick={() => void resolve(dispute, 'refund_buyer')} disabled={busy.includes(dispute.id) || remaining < 1}>Full refund</button></div></div>}
