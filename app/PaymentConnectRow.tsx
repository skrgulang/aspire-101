...
const copy: Record<PaymentStatus, { title: string; detail: string; action: string }> = {
  NOT_STARTED: { title: 'Seller payouts', detail: 'Connect a bank account securely with Stripe before selling with Aspire Protected.', action: 'Set up payouts' },
  ...
  READY: { title: 'Seller payouts ready ✓', detail: 'Stripe confirmed this account can receive Aspire seller payouts.', action: 'Manage bank & payouts' },
  ...
  return (
    <div className={`profileMenuRow paymentConnectRow payment-${status.toLowerCase()}`}>
      ...
      <div>
        ...
        <small>Bank account and routing details are entered and stored only with Stripe, never Aspire.</small>
