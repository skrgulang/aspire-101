...
export default function MarketplaceCheckoutV4() {
  ...
  const visibleItems = useMemo(() => {
    ...
    const next = items.filter((item) => {
      if (needle && !`${item.title} ${item.details || ''} ${item.category || ''}`.toLowerCase().includes(needle)) return false;
  ...
  async function reserve(item: MarketplaceItem) {
    ...
    try {
      ...
      if (deliveryChoice === 'aspirer') {
        ...
        if (!order?.id) throw new Error('The item was reserved, but Aspire could not attach the delivery request. Open Orders to continue.');
              ...
              return (
                <article className="marketV4Product" key={item.id}>
                  ...
                  <div className="marketV4Media">
                    ...
                    <div className="marketV4MediaTop">
                      <span>{item.id === directItemId ? 'Selected' : 'For sale'}</span>
                      {item.price_negotiable && <b>Negotiable</b>}
                    </div>
                  </div>
        ...
        <div className="marketV4Backdrop" role="dialog" aria-modal="true" aria-label="Choose how to receive this item" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setDeliveryFor(null); }}>
          <section className="marketV4Checkout">
            ...
            <div className="marketV4CheckoutHead">
              ...
              <div><p>BUY NOW · DELIVERY + PAYMENT</p><h2>How do you want to get it?</h2><span>{deliveryFor.title} · {money(itemAmount)}</span></div>
            </div>

            <div className="marketV4Choices">
              <button type="button" className={deliveryChoice === 'meet' ? 'active' : ''} disabled={!supports(deliveryFor, 'meet')} onClick={() => chooseDelivery('meet')}><div><b>Meet up</b><em>Delivery $0</em></div><span>Meet the seller on campus or nearby.</span></button>
            ...
            {deliveryChoice === 'aspirer' && <div className="marketV4Config"><h3>Set up the public delivery request</h3><div className="marketV4TwoCols"><label><span>Pickup area (public)</span><input value={pickupArea} onChange={(event) => setPickupArea(event.target.value)} placeholder="Seller / dorm area" /></label><label><span>Drop-off area shown publicly</span><input value={publicDropoffArea} onChange={(event) => setPublicDropoffArea(event.target.value)} placeholder="e.g. library area" /></label></div><div className="marketV4RewardRow">{(['free','5','10','negotiable'] as AspirerReward[]).map((reward) => <button type="button" key={reward} className={aspirerReward === reward ? 'active' : ''} onClick={() => setAspirerReward(reward)}>{reward === 'free' ? 'Free' : reward === '5' ? '$5' : reward === '10' ? '$10' : 'Negotiable'}</button>)}</div><p className="marketV4Fine">Only the general area is public. The exact delivery address stays private on the order.</p></div>}
