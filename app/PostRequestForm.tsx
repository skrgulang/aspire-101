...
export default function PostRequestForm() {
  ...
  return <>
    <form className="postForm" onSubmit={openConfirmation} noValidate>
      ...
      <section className={`requestMediaComposer ${isMarket ? 'isRecommended' : ''}`}><div className="requestMediaHead"><div><span>PHOTOS {isMarket && marketIntent === 'sell' ? '· REQUIRED FOR SELLERS' : isMarket ? '· RECOMMENDED' : '· OPTIONAL'}</span><strong>Show people what you mean.</strong></div><small>Up to 5 · JPG, PNG, WebP or HEIC · 8 MB each</small></div><div className="requestMediaRail">{photoPreviews.map((item, index) => <figure key={`${item.file.name}-${index}`}><img src={item.url} alt={`Request preview ${index + 1}`} /><button type="button" onClick={() => removePhoto(index)} aria-label={`Remove photo ${index + 1}`}>×</button>{index === 0 && <figcaption>COVER</figcaption>}</figure>)}{photos.length < 5 && <label className="requestMediaAdd"><input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={choosePhotos} /><i>+</i><strong>Add photos</strong><span>{photos.length}/5</span></label>}</div></section>
