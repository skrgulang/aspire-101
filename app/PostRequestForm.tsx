      ...
      {isMarket && (
        <section className="marketComposer" aria-label="Campus marketplace listing details">
          ...
          <div className="marketFields">
            ...
            <div className="marketFulfillment"><span>FULFILLMENT</span><div className="marketFulfillmentChoices"><button type="button" className={fulfillmentMethod === 'campus_pickup' ? 'active' : ''} onClick={() => setFulfillmentMethod('campus_pickup')}>Campus pickup</button><button type="button" className={fulfillmentMethod === 'shipping' ? 'active' : ''} onClick={() => setFulfillmentMethod('shipping')}>Carrier shipping</button></div><small>{fulfillmentMethod === 'shipping' ? 'After a match, seller and buyer add their own shipping details, compare enabled USPS / UPS / FedEx rates through Shippo, and the buyer locks a rate before payment.' : 'Meet in a sensible public campus location.'}</small><small>Need meetup + shipping + Aspirer delivery together? <a href="/marketplace/sell">Use the Flexible Fulfillment seller flow ↗</a></small></div>
          </div>
