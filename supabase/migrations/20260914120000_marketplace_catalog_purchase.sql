...
create index if not exists requests_marketplace_catalog_idx
  on public.requests (campus_id, created_at desc)
  where kind = 'buy_sell' and market_intent = 'sell' and status = 'open';
...
begin
  ...
  insert into public.connections (
    ...
    jsonb_build_object('category', v_request.category, 'kind', v_request.kind, 'source', 'marketplace_buy_now'),
    'aspire'
