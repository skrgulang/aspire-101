...
begin
  ...
  ) then raise exception 'LISTING_UNAVAILABLE';
  ...
  if v_fulfillment_method = 'shipping' then
    ...
    if v_shipping_policy not in ('buyer','seller','either') then v_shipping_policy := 'buyer'; end if;
    v_shipping_paid_by := coalesce(nullif(p_shipping_paid_by,''), case when v_shipping_policy='seller' then 'seller' else 'buyer' end);
    if v_shipping_paid_by not in ('buyer','seller') then raise exception 'INVALID_SHIPPING_PAYER'; end if;
