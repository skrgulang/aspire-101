-- Restore avatar updates through a narrow self-only RPC.
-- Generic browser UPDATE on profiles remains revoked.

create or replace function public.set_my_avatar_url(
  p_storage_path text,
  p_avatar_url text
)
returns void
...
begin
  ...
  if v_url = '' or char_length(v_url) > 2048 or v_url not like 'https://%' then
  ...
  v_expected_fragment := '/storage/v1/object/public/avatars/' || v_path;
  if position(v_expected_fragment in v_url) = 0 then
